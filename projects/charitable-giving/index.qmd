...
function initTheater() {
  ...
  function buildTimeline() {
    ...
    // SCENE 1: Character intro
    tl.call(function() {
      clearScreen();
      showCharacter('../../assets/c1.svg',
        'Let me tell you about an experiment that sent 50,000 letters to real donors.');
      fadeInScreen();
    })
    .to({}, { duration: 3 });

    // SCENE 2: Act 1, The Setup (envelopes)
    tl.call(function() {
      fadeOutScreen().then(function() {
        ...
        for (var i = 0; i < 24; i++) {
          ...
          envHtml += '<div class="gs-env" style="opacity:0;transform:scale(0);animation:gs-env-pop 0.3s ' + delay + 's forwards;"><div class="gs-env-flap"></div></div>';
        }
        screen.innerHTML = '<div class="gs-act-wrap">' +
          '<div class="gs-act-tag">Act 1</div>' +
          '<div class="gs-act-name">The Setup</div>' +
          '<div class="gs-env-grid">' + envHtml + '</div>' +
          '<div style="text-align:center;color:#888;font-style:italic;font-size:0.88rem;margin-top:16px;">50,000 letters, one charity, one question.</div>' +
          '</div>';
        fadeInScreen();
      });
    })
    ...
    // SCENE 3: Character comment
    tl.call(function() {
      fadeOutScreen().then(function() {
        clearScreen();
        showCharacter('../../assets/c2.svg',
          'Two-thirds of letters included a match offer. The rest did not.');
        fadeInScreen();
      });
    })
    .to({}, { duration: 2 });

    // SCENE 4: Act 2, The Match
    tl.call(function() {
      fadeOutScreen().then(function() {
        clearScreen();
        screen.innerHTML = '<div class="gs-act-wrap">' +
          '<div class="gs-act-tag">Act 2</div>' +
          '<div class="gs-act-name">The Match Offer</div>' +
          ...
          '</div>' +
          '<div style="text-align:center;color:#888;font-style:italic;font-size:0.88rem;margin-top:16px;">Donate $1. We add $1. Charity gets $2.</div>' +
          '</div>';
        fadeInScreen();
        gsap.fromTo('.gs-match-item', { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.4, stagger: 0.15, ease: 'back.out(1.3)', delay: 0.3 });
        gsap.fromTo('.gs-match-plus, .gs-match-equals', { scale: 0 }, { scale: 1, duration: 0.3, stagger: 0.1, ease: 'back.out(2)', delay: 0.6 });
      });
    })
    .to({}, { duration: 2 });
    ...
    tl.call(function() {
      fadeOutScreen().then(function() {
        clearScreen();
        showCharacter('../../assets/c1.svg',
          'Fundraisers swear bigger matches work harder. Is a 3-to-1 match three times as persuasive?');
        fadeInScreen();
      });
    })
    .to({}, { duration: 2 });

    // SCENE 6: Act 3, Three Ratios (coin stacks)
    tl.call(function() {
      fadeOutScreen().then(function() {
        clearScreen();
        ...
        screen.innerHTML = '<div class="gs-act-wrap">' +
          '<div class="gs-act-tag">Act 3</div>' +
          '<div class="gs-act-name">Three Ratios</div>' +
          '<div class="gs-ratio-grid">' +
            coinStack(1, 1, '1:1') +
            coinStack(1, 2, '2:1') +
            coinStack(1, 3, '3:1') +
          '</div>' +
          '<div style="text-align:center;color:#888;font-style:italic;font-size:0.88rem;margin-top:16px;">Three match ratios, randomly assigned.</div>' +
          '</div>';
        fadeInScreen();
        gsap.fromTo('.gs-ratio-col', { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, stagger: 0.2, ease: 'back.out(1.3)', delay: 0.3 });
        gsap.fromTo('.gs-coin-disc', { scale: 0 }, { scale: 1, duration: 0.2, stagger: 0.05, ease: 'back.out(2)', delay: 0.5 });
      });
    })
    .to({}, { duration: 3 });
    ...
    tl.call(function() {
      fadeOutScreen().then(function() {
        clearScreen();
        showCharacter('../../assets/c2.svg',
          'Let us see how many donors responded to each one.');
        fadeInScreen();
      });
    })
    .to({}, { duration: 2 });

    // SCENE 8: Act 4, The Response (three bars)
    tl.call(function() {
      fadeOutScreen().then(function() {
        clearScreen();
        screen.innerHTML = '<div class="gs-act-wrap">' +
          '<div class="gs-act-tag">Act 4</div>' +
          '<div class="gs-act-name">The Response</div>' +
          ...
          '<div style="text-align:center;color:#888;font-style:italic;font-size:0.88rem;margin-top:12px;">Response rates barely differ.</div>' +
          '</div>';
        fadeInScreen();
        var maxH = 3.0;
        for (var b = 1; b <= 3; b++) {
          (function(idx) {
            ...
            var counter = { v: 0 };
            gsap.to(bar, { height: (val / maxH * 100) + '%', duration: 1, ease: 'power2.out', delay: 0.3 + idx * 0.15 });
            gsap.to(counter, { v: val, duration: 1, ease: 'power2.out', delay: 0.3 + idx * 0.15,
              onUpdate: function() { pctEl.textContent = counter.v.toFixed(2) + '%'; }
            });
          })(b);
        }
        gsap.to('#gs-nolift', { opacity: 1, duration: 0.5, delay: 1.8 });
      });
    })
    .to({}, { duration: 3 });
    ...
    tl.call(function() {
      fadeOutScreen().then(function() {
        ...
        screen.innerHTML = '<div class="gs-char-scene">' +
          '<div class="gs-char-big"><img src="../../assets/c1.svg" alt="Guide" /></div>' +
          '<div class="gs-bubble"><div class="gs-bubble-tail"></div>' +
            '<div class="gs-verdict-big">Match works.</div>' +
            '<p>But bigger match ratios do not beat smaller ones. Scroll down for the full replication of the paper\'s findings.</p>' +
          '</div></div>';
        fadeInScreen();
