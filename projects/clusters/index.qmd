...
/* Fullscreen: image fills the available space, chrome rows stack below it in normal flow */
...
.cl-viewer:fullscreen .cl-chrome-row,
.cl-viewer:-webkit-full-screen .cl-chrome-row {
  position: static;
  flex: 0 0 auto;
  width: 100%;
  margin: 0;
...
.cl-viewer:fullscreen .cl-ctrl-btn,
.cl-viewer:-webkit-full-screen .cl-ctrl-btn {
  background: rgba(250,246,239,0.92);
  color: #2c3e50;
  border: 1px solid rgba(184,148,90,0.4);
}
.cl-viewer:fullscreen .cl-ctrl-btn:hover:not(:disabled),
.cl-viewer:-webkit-full-screen .cl-ctrl-btn:hover:not(:disabled) {
  background: #faf6ef;
}
.cl-viewer:fullscreen .cl-counter,
.cl-viewer:-webkit-full-screen .cl-counter {
  color: #faf6ef;
}
.cl-viewer:fullscreen .cl-sections-row,
.cl-viewer:-webkit-full-screen .cl-sections-row {
  position: static;
  flex: 0 0 auto;
  width: 100%;
  margin: 0;
  ...
  gap: 6px;
  background: rgba(28,44,58,0.95);
  box-sizing: border-box;
}
.cl-viewer:fullscreen .cl-section-btn,
.cl-viewer:-webkit-full-screen .cl-section-btn {
  border-color: rgba(184,148,90,0.5);
}
.cl-viewer:fullscreen .cl-section-label,
.cl-viewer:-webkit-full-screen .cl-section-label {
  color: #faf6ef;
  mix-blend-mode: normal;
}
.cl-viewer:fullscreen .cl-section-btn.has-fill .cl-section-fill,
.cl-viewer:-webkit-full-screen .cl-section-btn.has-fill .cl-section-fill {
  background: #b8945a;
}
...
/* Chrome row: notes pill on the left + controls cluster on the right (single row below the image) */
.cl-chrome-row {
  display: flex;
  align-items: center;
  gap: 16px;
...
/* Controls cluster (notes toggle, prev arrow, counter, next arrow) */
.cl-controls {
...
/* Section navigation row (centered pills) */
.cl-sections-row {
...
@media (max-width: 720px) {
  ...
  .cl-chrome-row { gap: 10px; padding: 0 6px; flex-wrap: wrap; }
  .cl-notes { font-size: 13px; padding: 9px 38px 9px 16px; max-height: 72px; }
  .cl-section-btn { padding: 5px 10px; }
  .cl-section-label { font-size: 11.5px; }
}
...
/* Persistent algorithm step list */
.kl-stepcard {
  flex: 1 1 320px;
  min-width: 260px;
  max-width: 380px;
...
.kl-stepcaption {
  margin: 0 0 10px;
  font-size: 13.5px; font-style: italic; color: #4a5563;
}
.kl-steps {
...
.kl-step-text { flex: 1 1 auto; }
.kl-step-iter {
  color: #b8945a; font-weight: 600; margin-left: 6px;
}
/* Active state: navy fill, cream text, pulsing brass dot */
.kl-step.active {
  background: #2c3e50; color: #faf6ef;
}
...
.kl-step.active .kl-step-icon::after {
  content: ""; position: absolute;
  width: 8px; height: 8px; border-radius: 50%;
  background: #b8945a;
  animation: kl-pulse 1.1s ease-in-out infinite;
}
.kl-step.active .kl-step-iter { color: #f5d488; }
/* Completed state: brass check, softer body text */
.kl-step.done { color: #2c3e50; opacity: 0.7; }
.kl-step.done .kl-step-icon {
  background: #b8945a; border-color: #b8945a;
}
.kl-step.done .kl-step-icon::after {
  content: ""; position: absolute;
...
.kl-step.done .kl-step-iter { color: #b8945a; opacity: 0.8; }
@keyframes kl-pulse {
  0%, 100% { transform: scale(0.6); opacity: 0.55; }
  50%      { transform: scale(1);   opacity: 1; }
}
/* Convergence banner (held until reset) */
.kl-converged {
  margin: 12px 0 0;
  padding: 10px 14px;
  background: #faf6ef;
  ...
  border-radius: 6px;
  color: #2c3e50;
  font-size: 14.5px; line-height: 1.45; font-style: italic;
  opacity: 0;
  transition: opacity 0.45s ease;
}
.kl-converged.visible { opacity: 1; }
.kl-converged strong { font-style: normal; font-weight: 600; color: #b8945a; }
...
.kl-widget:fullscreen .kl-stage-row,
.kl-widget:-webkit-full-screen .kl-stage-row {
  display: flex; flex-direction: column; flex-wrap: nowrap;
  flex: 1; min-height: 0; max-width: none; width: 100%;
  margin: 10px 0 0; gap: 10px;
}
...
.kl-widget:fullscreen .kl-stepcard,
.kl-widget:-webkit-full-screen .kl-stepcard {
  flex: 0 0 auto; max-width: 100%; min-width: 0;
  margin: 0; align-self: stretch;
}

@media (max-width: 920px) {
  .kl-stepcard { max-width: none; }
}
...
<div class="cl-viewer" id="cl-viewer">
  ...
  <div class="cl-chrome-row" id="cl-chrome-row">
  ...
  <div class="cl-sections-row" id="cl-sections"></div>
...
function initKmeansWidget() {
  ...
  function setStep(n, state) {
    // n is 1-based; state in {'idle','active','done'}
