...
TODO: write after seeing traces. Suggested observation: after the brass dashed line the chain explores a stable band around the MLE estimates, no upward or downward drift, no stretches of stuck identical values. The sampler is mixing well and the posterior summaries below are reliable.
...
TODO: write after seeing the side-by-side table. Suggested observations: with a weakly informative prior and N = 1,275 observed choices, the posterior means line up almost exactly with the MLE point estimates and the posterior SDs almost exactly with the MLE standard errors. This is the asymptotic equivalence of Bayes and MLE in action. The interesting payoff is not the betas themselves but the credible interval on the share of preference, which falls out of the posterior with no delta-method gymnastics. Marketing Analytics, for example, has a 95 percent credible interval on its share of roughly [24%, 32%], a range that is genuinely informative for anyone deciding which class to make a flagship.
