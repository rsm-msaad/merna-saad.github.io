...
<div class="ab-explainer">
  <div class="ab-step-header">
    <div class="ab-step-counter" id="step-counter">Step 1 of 5</div>
    <div class="ab-step-title" id="step-title">Visitors arrive at your site</div>
    <div class="ab-step-desc" id="step-desc">Each person lands on your homepage and sees one of two calls to action.</div>
  </div>

  <div class="ab-stage">
    <div class="ab-group ab-group-a">
      <div class="ab-label">CTA A</div>
      <div class="ab-subtitle">"Sign up for our newsletter here!"</div>
      <div class="ab-visitors" id="visitors-a"></div>
      <div class="ab-stats">
        <span class="ab-stat-label">Conversion rate</span>
        <span class="ab-stat-value" id="rate-a">0.0%</span>
      </div>
    </div>
    <div class="ab-divider"></div>
    <div class="ab-group ab-group-b">
      <div class="ab-label">CTA B</div>
      <div class="ab-subtitle">"Stay up to date by signing up!"</div>
      <div class="ab-visitors" id="visitors-b"></div>
      <div class="ab-stats">
        <span class="ab-stat-label">Conversion rate</span>
        <span class="ab-stat-value" id="rate-b">0.0%</span>
      </div>
    </div>
  </div>

  <div class="ab-verdict" id="ab-verdict" style="display: none;">
    <div class="ab-verdict-row">
      <span class="ab-verdict-label">Difference in rates</span>
      <span class="ab-verdict-value" id="verdict-diff">...</span>
    </div>
    <div class="ab-verdict-row">
      <span class="ab-verdict-label">95% Confidence interval</span>
      <span class="ab-verdict-value" id="verdict-ci">...</span>
    </div>
    <div class="ab-verdict-row">
      <span class="ab-verdict-label">p-value</span>
      <span class="ab-verdict-value" id="verdict-p">...</span>
    </div>
    <div class="ab-verdict-conclusion" id="verdict-conclusion"></div>
  </div>
  ...
  <div class="ab-controls">
    <button class="ab-btn ab-btn-ghost" id="btn-reset">Restart</button>
    <div class="ab-step-dots">
      <span class="ab-dot-step active" data-step="1"></span>
      <span class="ab-dot-step" data-step="2"></span>
      <span class="ab-dot-step" data-step="3"></span>
      <span class="ab-dot-step" data-step="4"></span>
      <span class="ab-dot-step" data-step="5"></span>
    </div>
    <button class="ab-btn ab-btn-primary" id="btn-next">Next</button>
  </div>
</div>
...
(function() {
  var steps = [
    {
      title: "Visitors arrive at your site",
      desc: "Each person lands on your homepage and sees one of two calls to action.",
      action: "intro"
    },
    {
      title: "Randomly assign each visitor to A or B",
      desc: "Half see CTA A. Half see CTA B. Random assignment ensures a fair comparison.",
      action: "assign"
    },
    {
      title: "Some visitors convert, others don't",
      desc: "A filled dot means they signed up. A faded dot means they didn't. Watch the conversion rates update live.",
      action: "convert"
    },
    {
      title: "Compare the two conversion rates",
      desc: "CTA A shows a higher rate. But is this difference real, or just random noise?",
      action: "compare"
    },
    {
      title: "Run the statistical test",
      desc: "We use a hypothesis test to check if the observed difference is statistically significant.",
      action: "verdict"
    }
  ];
  ...
  var piA = 0.22;
  var piB = 0.18;
  var targetVisitors = 80;
  var currentStep = 0;
  var animationId = null;

  var containerA = document.getElementById('visitors-a');
  var containerB = document.getElementById('visitors-b');
  ...
  var verdictBox = document.getElementById('ab-verdict');
  var dots = document.querySelectorAll('.ab-dot-step');
  ...
  function reset() {
    ...
    rateA.textContent = '0.0%';
    rateB.textContent = '0.0%';
    visitorsA = 0; convertsA = 0;
    visitorsB = 0; convertsB = 0;
  ...
  function addDot(container, type, delay) {
    setTimeout(function() {
      var dot = document.createElement('span');
      dot.className = 'ab-dot ' + type;
  ...
  function runAction(action) {
    ...
    if (action === 'intro') {
      ...
      verdictBox.style.display = 'none';
      for (var i = 0; i < 12; i++) {
        addDot(containerA, 'ab-dot-pending', i * 40);
        addDot(containerB, 'ab-dot-pending', i * 40 + 20);
      }
    }
      ...
      containerB.innerHTML = '';
      for (var i = 0; i < 15; i++) {
        addDot(containerA, 'ab-dot-assigned-a', i * 50);
        addDot(containerB, 'ab-dot-assigned-b', i * 50 + 25);
      }
    }
      ...
      if (visitorsA < targetVisitors) {
        while (visitorsA < targetVisitors) {
          var convA = Math.random() < piA;
          if (convA) convertsA++;
          visitorsA++;
          var dot = document.createElement('span');
          dot.className = 'ab-dot ' + (convA ? 'ab-dot-convert-a' : 'ab-dot-miss-a');
          containerA.appendChild(dot);
        }
        while (visitorsB < targetVisitors) {
          var convB = Math.random() < piB;
          if (convB) convertsB++;
          visitorsB++;
          var dot2 = document.createElement('span');
          dot2.className = 'ab-dot ' + (convB ? 'ab-dot-convert-b' : 'ab-dot-miss-b');
          containerB.appendChild(dot2);
        }
        rateA.textContent = ((convertsA / visitorsA) * 100).toFixed(1) + '%';
        rateB.textContent = ((convertsB / visitorsB) * 100).toFixed(1) + '%';
      }
      verdictBox.style.display = 'none';
    }

    else if (action === 'verdict') {
      var pA = convertsA / visitorsA;
      var pB = convertsB / visitorsB;
      var diff = pA - pB;
      var se = Math.sqrt((pA * (1 - pA)) / visitorsA + (pB * (1 - pB)) / visitorsB);
      var z = se > 0 ? diff / se : 0;
      var ciLow = diff - 1.96 * se;
      var ciHigh = diff + 1.96 * se;
      var p = 2 * (1 - normCdf(Math.abs(z)));

      document.getElementById('verdict-diff').textContent = (diff * 100).toFixed(1) + ' percentage points';
      document.getElementById('verdict-ci').textContent = '[' + (ciLow * 100).toFixed(1) + '%, ' + (ciHigh * 100).toFixed(1) + '%]';
      document.getElementById('verdict-p').textContent = p.toFixed(3);
      var concl = document.getElementById('verdict-conclusion');
      if (p < 0.05) {
        concl.textContent = 'p < 0.05: The difference is statistically significant. CTA A wins.';
        concl.className = 'ab-verdict-conclusion ab-verdict-win';
      } else {
        concl.textContent = 'p >= 0.05: Not enough evidence to call a winner. Keep testing.';
        concl.className = 'ab-verdict-conclusion ab-verdict-neutral';
      }
      verdictBox.style.display = 'block';
    }
  }

  function tickConvert() {
    if (visitorsA >= targetVisitors && visitorsB >= targetVisitors) return;
    if (visitorsA < targetVisitors) {
      var convA = Math.random() < piA;
      if (convA) convertsA++;
      visitorsA++;
    ...
    animationId = setTimeout(tickConvert, 40);
  }
