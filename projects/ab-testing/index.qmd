...
(function() {
  var narration = {
    1: "First, the question. We have two versions of a call to action on our website. One feels more direct, the other feels more benefit-driven. Our job is to find out which one gets more sign-ups.",
    2: "When a visitor arrives, we flip a coin. Heads they see Version A, tails they see Version B. This random assignment is the whole point: it rules out other explanations. Any difference we find has to be because of the CTA itself.",
    3: "Now we wait. Some visitors sign up, others don't. Each dot is a real person making a real choice. Watch how the sign-up rates converge toward their true values as more visitors arrive. This is the Law of Large Numbers in action.",
    4: "Version A looks like it's winning. But wait. Even if A and B were identical, we'd still see some difference just from random luck. The question is: how extreme is our observed difference? That's what the p-value tells us.",
    5: "Our p-value is small, meaning it would be very unlikely to see this big a difference if A and B were truly the same. So we reject that idea. A really does beat B. That's how an A/B test reaches a conclusion."
  };
  ...
  function typeText(el, text, speed) {
    ...
    function step() {
      if (i < text.length) {
        ...
        i++;
        animationTimers.push(setTimeout(step, speed || 20));
      }
    }
    step();
  }
  ...
  function showStep(n) {
    ...
    typeText(bubble, narration[n], 18);
      ...
      (function(idx) {
        animationTimers.push(setTimeout(function() {
          ...
          animationTimers.push(setTimeout(function() {
            ...
            newDot.className = 'ab-tiny-dot ' + targetClass + ' fly-in';
    ...
    for (var i = 0; i < total; i++) {
      (function(idx) {
        animationTimers.push(setTimeout(function() {
          ...
          rateA.textContent = Math.round((convertsA / visitorsA) * 100) + '%';
          ...
          rateB.textContent = Math.round((convertsB / visitorsB) * 100) + '%';
        }, idx * 50));
      })(i);
    }
  }
  ...
  function runScene4() {
    ...
    document.getElementById('sig-diff').textContent = '+' + (diff * 100).toFixed(1) + ' pp';
    document.getElementById('sig-p').textContent = p.toFixed(4);
  ...
  function runScene5() {
    ...
    document.getElementById('verdict-lift').textContent = '+' + lift.toFixed(1) + '%';
