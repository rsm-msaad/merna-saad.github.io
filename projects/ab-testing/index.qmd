...
(function() {
  ...
  function addVisitor(container, converted) {
    var dot = document.createElement('span');
    dot.className = 'ab-dot ' + (converted ? 'ab-dot-convert' : 'ab-dot-miss');
    container.appendChild(dot);
  }

  function tick() {
    if (visitorsA >= maxVisitors && visitorsB >= maxVisitors) return;

    if (visitorsA < maxVisitors) {
      var convA = Math.random() < piA;
      if (convA) convertsA++;
      visitorsA++;
      addVisitor(containerA, convA);
      rateA.textContent = ((convertsA / visitorsA) * 100).toFixed(1) + '%';
    }

    if (visitorsB < maxVisitors) {
      var convB = Math.random() < piB;
      if (convB) convertsB++;
      visitorsB++;
      addVisitor(containerB, convB);
